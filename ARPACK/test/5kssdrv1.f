...
c
      ...
      parameter       (maxn=10000000, maxnev=10, maxncv=(2*maxnev), 
     $                 ldv=maxn )
c
c     %--------------%
c     | Local Arrays |
...
c
      ...
      n = nx*nx
      nev =  3 
      ncv =  3*nev+1 !NCV Must be greater than at least 2*NEV
      if ( n .gt. maxn ) then
...
c     %------------%
c
      open(10,file='data.csv',access='sequential',
     &      form='formatted',iostat=res)
      
      do ix = 1, nx
           read(10,*) line
           print *, 'line',line !,'read.  d(ix,:)=', d(ix,:)
      end do
c
c
      print *, '   /n'
